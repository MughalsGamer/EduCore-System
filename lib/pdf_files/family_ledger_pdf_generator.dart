...
// // ─────────────────────────────────────────────────────────────────────────
// // Family Ledger PDF
// // Same visual language as the Employee Ledger PDF (navy header, purple
// // accents, summary boxes, striped table) — but:
// //   • No profile image (family has no single photo)
// //   • Every DEBIT row (a fee challan) expands into a compact student-wise
// //     fee breakdown sub-table directly beneath it, so a Rs 8,500 challan
// //     clearly shows which student contributed what and for which head
// //     (Admission / Annual / Monthly).
// //   • CREDIT rows (payments collected) render as single lines, same as
// //     the employee ledger.
// // ─────────────────────────────────────────────────────────────────────────
//
...
// /// Kept minimal and independent of any specific provider model so this
// /// generator only needs plain values passed in from wherever the
// /// collection data lives (FeeCollectionProvider / FamilyLedgerEntry / etc).
// class FamilyLedgerCreditEntry {
//   final DateTime date;
//   final double amount;
//   final String description; // e.g. "Fee Collection" / receipt no.
//   final String? note;
//
...
//   final bool isDebit;
//   final double amount;
//   final FeeChallanModel? challan; // set only for debit rows
//   final FamilyLedgerCreditEntry? credit; // set only for credit rows
...
//   required List<FeeChallanModel> challans, // debit side
//   required List<FamilyLedgerCreditEntry> credits, // credit side
// }) async {
...
//   //     compact student-wise breakdown table right underneath ──
...
//
//     if (!r.isDebit) {
//       // Credit rows: just the single line, no breakdown needed.
//       return summaryLine;
//     }
//
//     // Debit rows: append student-wise fee breakdown sub-table.
...
//           )
//         else ...[
//           // Sticky-looking table header (repeated as a normal block, since
//           // MultiPage doesn't support per-row expandable heights inside a
//           // native pw.Table when rows have variable content).
//           pw.Table(
//             columnWidths: const {
//               0: pw.FlexColumnWidth(1.6),
//               1: pw.FlexColumnWidth(3.6),
...
//                     pw.Align(
//                       alignment: pw.Alignment.centerRight,
//                       child: pw.Text('${_fmtMoney(balance.abs())} ${balance >= 0 ? 'Dr' : 'Cr'}',
