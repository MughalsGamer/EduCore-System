...
class SalaryPdfService {
  ...
  /// Builds the salary slip PDF document bytes for a single record.
  static Future<Uint8List> _buildPdf(SalaryRecord rec) async {
    final doc = pw.Document();
    doc.addPage(
      pw.Page(
        pageFormat: PdfPageFormat.a4,
        margin: const pw.EdgeInsets.all(25),
        build: (context) => _buildPageContent(rec),
      ),
    );
    return doc.save();
  }

  /// Merges multiple salary slips into a single multi-page PDF.
  static Future<Uint8List> buildMergedPdf(List<SalaryRecord> records) async {
    final doc = pw.Document();
  ...
  /// Shared page content builder – used by both single and bulk PDFs.
  static pw.Widget _buildPageContent(SalaryRecord rec) {
    const int totalDays = 30;
    final int presentDays = totalDays - rec.leaves;
    ...
    final int absentDays = rec.leaves;
    final double totalEarnings = rec.baseSalary + rec.bonus;
    final double totalDeductions = rec.absentDeduction +
        rec.fine +
        (rec.recordInLedger ? rec.ledgerDeductionAmount : 0);
    final double netPayable = rec.payableNetSalary;
    ...
    return pw.Column(
      ...
      children: [
        ...
        pw.SizedBox(height: 8),
        _buildSalaryBreakdownTable(
            rec, presentDays, absentDays, totalEarnings, totalDeductions, fmt),
        pw.SizedBox(height: 12),
        _buildNetPayableBox(netPayable, fmt),
        pw.SizedBox(height: 20),
        if (rec.note != null && rec.note!.trim().isNotEmpty) ...[
        ...
        child: pw.Row(
          ...
          children: [
            pw.SizedBox(
              width: 95,
              child:
              pw.Text(label, style: pw.TextStyle(fontSize: 9.5, color: _kSlate)),
            ),
            pw.Text(':', style: pw.TextStyle(fontSize: 9.5, color: _kSlate)),
            pw.SizedBox(width: 8),
            pw.Expanded(
              ...
              infoBlock(
                  ...
                  pw.Text(
                      DateFormat('MMMM yyyy')
                          .format(DateTime(rec.year, rec.month)),
                      style: pw.TextStyle(fontSize: 9.5, color: _kInk))),
              pw.SizedBox(height: 12),
              infoBlock(
                  'PAYROLL PERIOD',
  ...
  static pw.Widget _buildSalaryBreakdownTable(SalaryRecord rec, int present,
      int absent, double earnings, double deductions,
      String Function(num) fmt) {
    pw.Widget row(String desc, String amount,
        {bool isTotal = false, bool isDeduction = false, bool isAdd = false}) {
      return pw.Container(
        padding: const pw.EdgeInsets.symmetric(horizontal: 8, vertical: 6),
        decoration: pw.BoxDecoration(
          color: isTotal ? _kRowGrey : null,
          border: const pw.Border(
              bottom:
              pw.BorderSide(color: PdfColor.fromInt(0xFFEDEFF2), width: 0.5)),
        ),
        child: pw.Row(
          children: [
            pw.Expanded(
    ...
    return pw.Column(
      children: [
        ...
        row('Absent Days ($absent Days)', '-'),
        if (absent > 0)
          row('Absent Deduction ', '-${fmt(rec.absentDeduction)}',
              isDeduction: true),
        if (rec.fine > 0)
          row('Fine / Deduction', '-${fmt(rec.fine)}', isDeduction: true),
        if (rec.bonus > 0)
          row('Bonus / Addition (Performance Bonus)', '+${fmt(rec.bonus)}',
              isAdd: true),
        if (rec.recordInLedger && rec.ledgerDeductionAmount != 0)
          row('Balance Deduction', '-${fmt(rec.ledgerDeductionAmount)}',
              isDeduction: true),
        row('TOTAL Salary', fmt(earnings), isTotal: true),
        row('TOTAL DEDUCTIONS', fmt(deductions.abs()),
            isTotal: true, isDeduction: true),
      ],
    );
  }

  static pw.Widget _buildNetPayableBox(
      double netPayable, String Function(num) fmt) {
    return pw.Container(
      width: double.infinity,
      padding: const pw.EdgeInsets.symmetric(horizontal: 12, vertical: 10),
      decoration: pw.BoxDecoration(
      ...
      child: pw.Row(
        ...
        children: [
          pw.Column(
            crossAxisAlignment: pw.CrossAxisAlignment.start,
            children: [
              pw.Text('NET PAYABLE SALARY',
                  style: pw.TextStyle(
                      fontSize: 12,
                      fontWeight: pw.FontWeight.bold,
                      color: _kInk)),
              pw.Text('(Payable Amount)',
                  style: pw.TextStyle(fontSize: 9, color: _kSlate)),
            ],
          ),
          pw.Text(
            'Rs ${fmt(netPayable)}',
            style: pw.TextStyle(
                fontSize: 22, fontWeight: pw.FontWeight.bold, color: _kNavy),
          ),
        ],
      ),
    );
  ...
  static pw.Widget _buildNotes(String note) {
    return pw.Column(
      ...
      children: [
        pw.Text('NOTES',
            style: pw.TextStyle(
                fontSize: 10, fontWeight: pw.FontWeight.bold, color: _kInk)),
        pw.SizedBox(height: 4),
        pw.Row(
          crossAxisAlignment: pw.CrossAxisAlignment.start,
          children: [
            ...
            pw.Expanded(
              child: pw.Text(note,
                  style: pw.TextStyle(
                      fontSize: 9.5, color: _kSlate, height: 1.4)),
            ),
          ],
        ),
      ],
