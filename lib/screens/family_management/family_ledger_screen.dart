...
class _FamilyLedgerScreenState extends State<FamilyLedgerScreen> {
  ...
  // FIX (scroll-stuck bug): the ledger list on mobile used to live inside a
  // Container with a fixed maxHeight + its own Scrollbar/ListView, nested
  // inside the outer SingleChildScrollView. Two independent scrollables like
  // that fight each other — once the inner list finishes scrolling, the
  // gesture doesn't hand off to the outer scroll view, so the screen feels
  // "stuck" at the bottom of the ledger card. Now everything lives in ONE
  // scrollable (the outer SingleChildScrollView); the ledger list itself is
  // shrink-wrapped and non-scrolling so it just flows as part of the page.
  Widget _buildMobile() {
    return SingleChildScrollView(
      padding: const EdgeInsets.all(16),
      child: Column(
  ...
  Widget _summaryCardsRow({required bool isDesktop}) {
    ...
    // FIX (RenderFlex overflow): childAspectRatio: 1.5 made each grid cell
    // too short for the icon + label + value + badge/sub stack once real
    // data (longer labels, badge text) was laid out, causing the reported
    // 16px bottom overflow inside _summaryCard's Column. Lowering the aspect
    // ratio gives the cards a bit more height, and _summaryCard itself is
    // now defensive (Flexible/mainAxisSize.min) so it never overflows even
    // if the cell is tight.
    return GridView.count(
      crossAxisCount: 2,
      shrinkWrap: true,
      physics: const NeverScrollableScrollPhysics(),
  ...
  Widget _summaryCard({
    ...
    return Container(
      ...
      // FIX: mainAxisSize.min + LayoutBuilder-free Flexible text prevents
      // this Column from demanding more vertical space than its parent
      // (the GridView cell) actually has, which is what caused the 16px
      // RenderFlex overflow reported at line ~494.
      child: Column(
  ...
  // ── Desktop table with expandable rows (unchanged: desktop already has
  //    a bounded-height card so its own inner scroll is fine there) ──
  Widget _desktopTable(List<FamilyLedgerEntry> rows) {
    final provider = context.read<FeeCollectionProvider>();
    final totalDebit = provider.ledgerTotalDebit;
    final totalCredit = provider.ledgerTotalCredit;
  ...
  // ── Mobile cards list (FIXED: no more inner fixed-height scrollable) ──
  // Previously this wrapped everything in `Container(constraints:
  // BoxConstraints(maxHeight: 520))` + its own `Scrollbar`/`ListView`,
  // stacked inside the page's outer `SingleChildScrollView`. Two nested
  // scrollables meant the inner one absorbed the drag gesture and never
  // handed off to the outer one once it reached its own end — that's the
  // "scroll gets stuck, page won't go further" bug.
  //
  // Fix: build the rows as a plain shrink-wrapped, non-scrolling ListView.
  // There is now only ONE scrollable on the mobile screen (the outer
  // SingleChildScrollView in _buildMobile), so scrolling to the very
  // bottom — including the totals card below the list — works smoothly.
  Widget _mobileCardsList(List<FamilyLedgerEntry> rows) {
    final provider = context.read<FeeCollectionProvider>();
    return Column(
      children: [
        ListView.builder(
          ...
          itemBuilder: (context, index) {
            final entry = rows[index];
            final isExpanded = _expandedEntryIndex == index;
            final challanFuture = entry.type == 'debit'
                ? _getChallanFutureForEntry(entry, index)
                : null;
              ...
              child: Column(
                children: [
                  GestureDetector(
                    onTap: () {
                      setState(() {
                        _expandedEntryIndex = isExpanded ? null : index;
                      });
                    },
                    child: _mobileRow(entry, isExpanded: isExpanded),
                  ),
                  if (isExpanded && entry.type == 'debit')
                    _challanBreakdownFuture(challanFuture!),
                ],
