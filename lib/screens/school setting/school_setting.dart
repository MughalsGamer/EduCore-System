...
//           toolbarHeight: 0, // hides the AppBar completely
//         ),
//         body: body,
//       );
//     }
...
class _SchoolSettingsScreenState extends State<SchoolSettingsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    // Always use a Scaffold to provide Material context.
    // Conditionally set the AppBar.
    AppBar? appBar;
    if (widget.showAppBar) {
      appBar = AppBar(
        titleSpacing: 20,
      ...
      // When no app bar requested, show a back arrow and a minimal title.
      appBar = AppBar(
        automaticallyImplyLeading: true,
        title: const Text(
          'Settings',
          style: TextStyle(fontWeight: FontWeight.w600, fontSize: 16, color: _kInk),
        ),
        backgroundColor: _kCard,
        surfaceTintColor: _kCard,
        foregroundColor: _kInk,
  ...
  Widget _buildUsersTable(List<AppUser> users) {
    // Wrap with horizontal scroll to be safe on small desktops
    return SingleChildScrollView(
      scrollDirection: Axis.horizontal,
      child: Container(
        decoration: BoxDecoration(
  ...
  // ── Mobile stacked cards (password below role) ──
  Widget _buildUsersMobileList(List<AppUser> users) {
    return Column(
      children: List.generate(users.length, (i) {
        final u = users[i];
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 10),
              // Role and status on one line, password below
              Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Row(
                  ...
                  Row(
                    children: [
                      const Text('Password: ', style: TextStyle(fontSize: 12, color: _kSlate)),
                      _PasswordReveal(password: u.password),
                    ],
                  ),
                ],
