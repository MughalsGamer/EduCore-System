...
const _kPurple = Color(0xFF6D28D9);
const _kPurpleBg = Color(0xFFF5F3FF);
// ★ Neutral grey tone used for the Sunday/holiday state.
...
const List<Map<String, Object>> _kStatuses = [
  ...
  // ★ NEW — Sunday's forced status. Included here so the status
  // dropdown/badge lookups (_kStatuses.firstWhere) resolve correctly
  // instead of falling back to 'Present' when a Sunday record is shown.
  {
    'key': 'holiday',
    'label': 'Holiday',
...
class _AttendanceScreenState extends State<AttendanceScreen> with RouteAware {
  ...
  Widget _buildDesktopLayout(BuildContext context,
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          _buildToolbarCard(context, provider, isDesktop: true),
          // ★ NEW — Sunday banner, matches the "already saved" style
          // but explains the day is a locked holiday, not just saved.
          if (provider.isSelectedDateSunday) ...[
            const SizedBox(height: 10),
            _buildSundayBanner(),
          ],
          const SizedBox(height: 14),
  ...
  Widget _buildMobileLayout(
    ...
    return Column(
      children: [
        ...
        // ★ NEW — Sunday banner on mobile too.
        if (provider.isSelectedDateSunday) ...[
          const SizedBox(height: 10),
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 16),
            child: _buildSundayBanner(),
          ),
        ],
        const SizedBox(height: 10),
        Padding(
  ...
  // ★ NEW — Small info banner shown whenever the selected date is a
  // Sunday: explains why every row is locked/grey instead of leaving
  // the user to guess. Mirrors _ViewOnlyBanner's style used elsewhere
  // in the attendance module.
  Widget _buildSundayBanner() {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10),
      decoration: BoxDecoration(
        color: _kGreyBg,
        borderRadius: BorderRadius.circular(8),
        border: Border.all(color: _kGrey.withOpacity(0.2)),
      ),
      child: Row(
        children: const [
          Icon(Icons.info_outline, size: 16, color: _kGrey),
          SizedBox(width: 8),
          Expanded(
            child: Text(
              'Sunday is a holiday. Attendance cannot be marked for this date.',
              style: TextStyle(
                  fontSize: 12.5, color: _kGrey, fontWeight: FontWeight.w600),
            ),
          ),
        ],
      ),
  ...
  Widget _buildDatePicker(BuildContext context, AttendanceProvider provider) {
    final isSunday = provider.isSelectedDateSunday;
    return InkWell(
      key: _dateChipKey,
      borderRadius: BorderRadius.circular(8),
      onTap: () => _toggleDateOverlay(context, provider),
      child: Container(
        padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 12),
        decoration: BoxDecoration(
          // ★ NEW — subtle grey tint on the date chip itself when a
          // Sunday is selected, so the lock reads at a glance.
          color: isSunday ? _kGreyBg : _kSurface,
          borderRadius: BorderRadius.circular(8),
          border: Border.all(color: isSunday ? _kGrey.withOpacity(0.3) : _kBorder),
        ),
        child: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            Icon(Icons.calendar_today_outlined,
                size: 16, color: isSunday ? _kGrey : _kSlate),
            const SizedBox(width: 10),
            Flexible(
              child: Text(
                DateFormat('EEE, dd MMM yyyy')
                ...
                style: TextStyle(
                  fontSize: 13.5,
                  fontWeight: FontWeight.w600,
                  color: isSunday ? _kGrey : _kInk,
                ),
              ),
            ),
            const SizedBox(width: 6),
            Icon(Icons.keyboard_arrow_down, size: 18, color: isSunday ? _kGrey : _kSlate),
          ],
        ),
      ),
    );
  ...
  // that are already saved, and warn the user via a SnackBar if one or
  // more people were skipped because their attendance was already
  // marked for the selected date. On Sunday the whole row of buttons
  // is disabled and shows a "day is a holiday" message instead.
  Widget _buildQuickActions(BuildContext context, AttendanceProvider provider,
      {bool fullWidth = false}) {
    final isSunday = provider.isSelectedDateSunday;
  ...
  // ★ CHANGED — onTap is now nullable so the button can render as
  // disabled (locked) on Sunday instead of being fully hidden, keeping
  // the toolbar's layout stable.
  Widget _buildQuickActionBtn(String label, IconData icon, Color color,
      Color bgColor, VoidCallback? onTap) {
    final disabled = onTap == null;
    final effectiveColor = disabled ? color.withOpacity(0.4) : color;
    final effectiveBg = disabled ? bgColor.withOpacity(0.5) : bgColor;
  ...
  Widget _buildTableDataRow({
    ...
    final bool isSunday = provider.isSelectedDateSunday;
    // ★ CHANGED — Sunday forces the locked state regardless of the
    // record's own isSaved flag (though the provider already sets
    // isSaved: true for Sunday, this keeps the row logic explicit).
    final bool isSaved = isSunday || record.isSaved == true;
    final String? designation =
    (record.designation != null && (record.designation as String).trim().isNotEmpty)
        ? record.designation as String
        : null;
    ...
    void notifyAlreadySaved() {
      final message = isSunday
          ? 'Sunday is a holiday. Attendance cannot be marked.'
          : 'Attendance already marked for ${record.staffName} on this date.';
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
  ...
  Widget _buildMobileCompactRow({
    ...
    final bool isSunday = provider.isSelectedDateSunday;
    // ★ CHANGED — same Sunday-forced lock as the desktop table row.
    final bool isSaved = isSunday || record.isSaved == true;
    final String? designation =
    (record.designation != null && (record.designation as String).trim().isNotEmpty)
        ? record.designation as String
  ...
  // SAVE BAR
  // ============================================================
  // ★ CHANGED — the whole Save button is disabled on Sunday (nothing
  // to write to Firestore, matching class attendance where Sunday
  // never gets a doc), with its label swapped to explain why.
  Widget _buildSaveBar(BuildContext context, AttendanceProvider provider,
      {required bool isDesktop}) {
    final isSunday = provider.isSelectedDateSunday;

    final button = ElevatedButton.icon(
      onPressed: (provider.loading || isSunday)
          ? null
          : () async {
        await provider.saveAttendance();
        if (context.mounted) {
      ...
      icon: Icon(isSunday ? Icons.home_rounded : Icons.check_circle_outline, size: 17),
      label: Text(isSunday ? 'Sunday — Holiday' : 'Save Attendance',
          style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14)),
      style: ElevatedButton.styleFrom(
        backgroundColor: isSunday ? _kGrey : _kPrimary,
        foregroundColor: Colors.white,
        disabledBackgroundColor: _kGrey.withOpacity(0.5),
        padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 15),
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),
        elevation: 0,
      ),
...
class _StatusDropdown extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return PopupMenuButton<String>(
      ...
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
      onSelected: onChanged,
      itemBuilder: (ctx) => _kStatuses
          .where((opt) => opt['key'] != 'holiday') // ★ NEW — holiday is a system-only status, never user-selectable
          .map((opt) {
        final optKey = opt['key'] as String;
        final optLabel = opt['label'] as String;
        final optIcon = opt['icon'] as IconData;
