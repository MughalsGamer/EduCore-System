...
//       physics: const AlwaysScrollableScrollPhysics(), // Ensures scrolling even when content is small
//       padding: const EdgeInsets.all(12),
//       child: Column(
//         crossAxisAlignment: CrossAxisAlignment.start,
//         children: [
...
//   }
//
//   // Mobile Calendar – now uses shrinkWrap and no internal scrolling so parent scrolls
...
//           _weekDayRow(),
//           const SizedBox(height: 6),
//           // Removed fixed height – let it size itself
//           AnimatedSwitcher(
//             duration: const Duration(milliseconds: 200),
//             child: _calendarLoading
//                 ? const Center(child: CircularProgressIndicator(strokeWidth: 2))
...
//   }
//
//   // Mobile calendar grid – shrinkWrap = true, no scrolling physics
...
//       // CRITICAL FIX: prevent internal scrolling, shrink to fit content
//       shrinkWrap: true,
//       physics: const NeverScrollableScrollPhysics(),
//       children: cells,
//     );
//   }
//
//   // Mobile day cell – simpler, shows date and status indicator
...
class _BulkAttendanceScreenState extends State<BulkAttendanceScreen>
  ...
  // ─── Responsive Shimmer Skeleton (Desktop + Mobile) ──────────────────
  Widget _buildCalendarSkeleton() {
    final isDesktop = MediaQuery.of(context).size.width >= 900;
    return Shimmer.fromColors(
      baseColor: Colors.grey.shade200,
  ...
  Widget _skeletonBox({double? width, double? height, double radius = 8, Color? color}) {
    return Container(
      width: width,
      height: height,
      decoration: BoxDecoration(
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          Row(
            children: [
              _skeletonBox(width: 160, height: 40, radius: 8),
              const SizedBox(width: 12),
              _skeletonBox(width: 110, height: 40, radius: 8),
              const SizedBox(width: 8),
              _skeletonBox(width: 110, height: 40, radius: 8),
              const Spacer(),
              ...List.generate(6, (_) => Padding(
                padding: const EdgeInsets.only(left: 6),
                child: _skeletonBox(width: 70, height: 24, radius: 8),
              )),
            ],
          ),
          const SizedBox(height: 12),
          Expanded(
            child: Row(
              ...
              children: [
                ...
                Expanded(
                  flex: 30,
                  child: _buildCalendarCardSkeleton(),
                ),
                const SizedBox(width: 16),
                ...
                const SizedBox(width: 16),
                // Employee panel skeleton
                Expanded(
                  flex: 15,
                  child: _buildEmployeePanelSkeleton(),
                ),
              ],
            ),
          ),
  ...
  Widget _buildCalendarCardSkeleton() {
    return Container(
      ...
      child: Column(
        ...
        children: [
          // Dark header
          _skeletonBox(height: 36, radius: 8, color: _kPrimaryDark),
          const SizedBox(height: 10),
          // Day names row
          Row(
            children: List.generate(7, (_) => Expanded(
              child: Center(
                child: _skeletonBox(width: 24, height: 12, radius: 4),
              ),
            )),
          ),
          const SizedBox(height: 6),
          ...
          Expanded(
            child: GridView.count(
              ...
              childAspectRatio: 1.2,
              physics: const NeverScrollableScrollPhysics(),
              children: List.generate(42, (_) => _skeletonBox(radius: 6)),
            ),
          ),
          const SizedBox(height: 8),
          // Legend row
          Row(
            ...
            children: List.generate(6, (_) => _skeletonBox(width: 50, height: 12, radius: 4)),
          ),
        ],
      ),
    );
  ...
  Widget _buildOverviewCardSkeleton() {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _skeletonBox(width: 120, height: 15, radius: 4),
          const SizedBox(height: 16),
          ...
          Wrap(
            spacing: 8,
            runSpacing: 6,
            children: List.generate(6, (_) => _skeletonBox(width: 60, height: 10, radius: 2)),
          ),
        ],
      ),
    );
  ...
  Widget _buildEmployeePanelSkeleton() {
    return Container(
      decoration: BoxDecoration(
        color: _kCard,
        borderRadius: BorderRadius.circular(12),
        ...
        children: [
          Padding(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                _skeletonBox(width: 100, height: 15, radius: 4),
                const SizedBox(height: 10),
                _skeletonBox(height: 40, radius: 8),
              ],
            ),
          ),
          const Divider(height: 1, color: _kBorder),
          // List of employees placeholder
          Expanded(
            child: ListView.builder(
              physics: const NeverScrollableScrollPhysics(),
              itemCount: 6,
        ...
        children: [
          ...
          Row(
            ...
            children: [
              ...
              _skeletonBox(width: 150, height: 40, radius: 8),
            ],
          ),
        ],
      ),
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          _skeletonBox(width: double.infinity, height: 44, radius: 8),
          const SizedBox(height: 12),
          // Title skeleton
          _skeletonBox(width: 150, height: 20, radius: 4),
          const SizedBox(height: 12),
          // Search bar skeleton
          _skeletonBox(width: double.infinity, height: 44, radius: 8),
          const SizedBox(height: 10),
          ...
          SizedBox(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) => Padding(
                padding: const EdgeInsets.only(right: 8),
                child: _skeletonBox(width: 100, height: 42, radius: 8),
              ),
            ),
          ),
          const SizedBox(height: 12),
          ...
          Container(
            ...
            child: Column(
              children: [
                ...
                Row(
                  children: List.generate(7, (_) => Expanded(
                    child: Center(child: _skeletonBox(width: 24, height: 12, radius: 4)),
                  )),
                ),
                const SizedBox(height: 6),
                // Grid
                GridView.count(
                  ...
                  shrinkWrap: true,
                  physics: const NeverScrollableScrollPhysics(),
                  children: List.generate(42, (_) => _skeletonBox(radius: 8)),
                ),
                const SizedBox(height: 8),
                // Status legend
                Row(
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                _skeletonBox(width: 120, height: 14, radius: 4),
                const SizedBox(height: 12),
                _skeletonBox(height: 180, radius: 8),
              ],
            ),
          ),
          const SizedBox(height: 12),
          ...
          GridView.count(
            ...
            crossAxisSpacing: 8,
            childAspectRatio: 2.2,
            children: List.generate(6, (_) => _skeletonBox(height: 60, radius: 8)),
          ),
          const SizedBox(height: 8),
          // Total days
          _skeletonBox(width: 100, height: 12, radius: 4),
