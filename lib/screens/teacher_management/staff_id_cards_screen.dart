...
//
//               // ✅ Desktop/Tablet: EXACT same design/behaviour as before (untouched)
//               if (!isMobile) {
...
//               }
//
//               // ✅ Mobile: 2 columns, whole card proportionally scaled down
//               // (via FittedBox below) to fit each column's width, then
//               // scroll for more rows. Aspect ratio stays identical to the
//               // card's real design ratio, so FittedBox scales it cleanly.
...
//                 itemCount: filteredStaff.length,
//                 itemBuilder: (context, index) {
//                   // Fixed-size card rendered at full (design) size in an
//                   // unconstrained OverflowBox (so its internal Column never
//                   // overflows), then the whole rendered result is scaled
//                   // down by FittedBox to fit the smaller mobile cell —
//                   // keeps every font/icon/padding proportion identical to
//                   // desktop, just smaller, with no layout overflow.
//                   return FittedBox(
//                     fit: BoxFit.contain,
...
//   final double size;
//   const _TransparentLogo(this.settings, {this.size = 60});
//
//   @override
//   Widget build(BuildContext context) {
...
//     final roleText = (staff.designation != null && staff.designation!.trim().isNotEmpty)
...
//                 Container(
//                   constraints: const BoxConstraints(maxWidth: _kCardWidth - 60),
...
//           const Spacer(),
//
//           // ─── BOTTOM BLACK FOOTER (address, same as front card) ───
//           ClipPath(
//             clipper: _FooterClipper(),
//             child: Container(
//               height: 50,
...
class _StaffIdCardsScreenState extends State<StaffIdCardsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    // ─────────────────────────────────────────────────────────
    // FIX: When this screen is opened WITHOUT its own AppBar
    // (showAppBar == false — e.g. pushed as a standalone mobile
    // route), it used to just return `Scaffold(body: body)` with
    // no SafeArea and no way to go back. On mobile that meant the
    // content could sit under the status bar, and there was no
    // back arrow to return to the previous screen.
    //
    // Now we wrap everything in a SafeArea and add a small header
    // row with a back button + title, so mobile users always have
    // a safe top inset and a way to navigate back.
    // ─────────────────────────────────────────────────────────
    ...
    return Scaffold(
      ...
      // FIX: wrapped body in SafeArea too, so bottom gesture-bar / notch
      // areas on mobile never overlap the grid content.
