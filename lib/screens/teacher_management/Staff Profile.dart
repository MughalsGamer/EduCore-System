...
//   }
//
//   // ★ FIX: Complete history with all events - Joined → Terminated → Rejoined → Terminated → ...
...
//     }
//
//     // Ensure 'terminated' exists if staff is terminated
...
//     }
//
//     // Also check if terminationDate exists but wasn't in history
...
//     }
//
//     // ★ NEW: Check for rejoining events
//     // If staff is active and was previously terminated, ensure there's a rejoined event
...
//
//       // If there are terminated events but no rejoined events, add rejoined
//       // This handles reactivation through deactivate/rejoin flow
//       if (terminatedEvents.isNotEmpty && rejoinedEvents.isEmpty) {
//         // Find the last termination date to use as reference
//         terminatedEvents.sort((a, b) => b.date.compareTo(a.date));
//         final lastTerminationDate = terminatedEvents.first.date;
//
//         // Use the last termination date as rejoining date (or current date)
//         // This ensures the timeline shows termination → rejoining correctly
//         String rejoiningDate = DateTime.now().toIso8601String().split('T').first;
//
//         // Only add if rejoining date is after last termination
...
//     }
//
//     // ★ NEW: If there are multiple termination events, make sure each has a rejoined before it
//     // This handles multiple cycles: Join → Terminate → Rejoin → Terminate → Rejoin
//     final terminatedList = events.where((e) => e.type == 'terminated').toList();
//     final rejoinedList = events.where((e) => e.type == 'rejoined').toList();
//
//     // If we have more terminations than rejoins + 1 (the last termination might not have a rejoining yet)
//     // This ensures each termination (except the last if currently terminated) has a rejoining after it
...
//
//       // For each termination except the last one, ensure there's a rejoining after it
...
//
//         if (!hasRejoinAfter) {
//           // Find the next termination date to use as rejoining date
//           final nextTerm = terminatedList[i + 1];
//           // Use a date between term and next term
//           try {
...
//
//             // Only add if date is valid and between the two termination dates
