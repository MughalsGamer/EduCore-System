...
/// Shows BOTH deactivated Teachers and deactivated Staff on a single page.
...
/// - Reactivate action restores the member back to the active list
...
class DeactivatedStaffScreen extends StatefulWidget {
  /// 'teacher' | 'staff' | null (null / 'all' shows both)
  final String? initialTypeFilter;

  const DeactivatedStaffScreen({super.key, this.initialTypeFilter});

  @override
  State<DeactivatedStaffScreen> createState() =>
      _DeactivatedStaffScreenState();
}

class _DeactivatedStaffScreenState extends State<DeactivatedStaffScreen> {
  ...
  void _confirmReactivate(BuildContext context, StaffMember m) {
    showDialog(
        context: context,
        builder: (ctx) => AlertDialog(
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14)),
          title: const Text('Reactivate this member?',
              style: TextStyle(fontWeight: FontWeight.w600)),
          content: Text('"${m.name}" will be moved back to the active ${m.type == 'teacher' ? 'Teachers' : 'Staff'} list.'),
          actions: [
            TextButton(
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                Navigator.pop(ctx);
                await context.read<StaffProvider>().reactivateStaff(m.id!);
                if (context.mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text('${m.name} has been reactivated')),
                  );
                  setState(() {});
                }
              },
  ...
  Widget _buildDesktop() {
    ...
    final deactivated = provider.deactivatedMembers;
    // scope used to build section chips = type-filtered + class-filtered list
    var scopeForSections = deactivated;
    if (_typeFilter != 'all') {
      scopeForSections =
    ...
    final filtered = _applyFilters(deactivated);
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            ...
            Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('Deactivated (${filtered.length})',
                  style: const TextStyle(
                      fontSize: 22,
                      fontWeight: FontWeight.bold,
                      color: Color(0xFF1A1A2E))),
              const SizedBox(height: 2),
              Text('Deactivated teachers & staff',
  ...
  Widget _desktopRow(BuildContext context, StaffMember m) {
    ...
    return Padding(
        ...
        child: Row(children: [
          ...
          Expanded(
            flex: 13,
            child: Text(m.designation?.isNotEmpty == true ? m.designation! : '—',
          ...
          Expanded(
            flex: 10,
            child: Center(
              child: _actionBtn(Icons.restart_alt, const Color(0xFF15803D),
                      () => _confirmReactivate(context, m),
                  tooltip: 'Reactivate'),
            ),
          ),
        ]));
