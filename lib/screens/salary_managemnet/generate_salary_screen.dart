...
class _GenerateSalaryScreenState extends State<GenerateSalaryScreen> {
  ...
  void _hydrateSelectedEmployeeForEdit() {
    ...
    final staffProvider = context.read<StaffProvider>();
    // search allStaff too, since a terminated employee is
    ...
    // this salary was generated (recordInLedger / ledgerDeductionAmount
    // are now persisted on SalaryRecord). Without this, the edit screen
    // always opened with the toggle OFF, even if a deduction had been
  ...
  Future<void> _fetchLedgerBalance() async {
    ...
    try {
      ...
      // In edit mode, subtract back out this record's own linked credit
      // transaction (if any), so the base balance is "as if" this
      // salary's deduction hadn't been applied yet.
      if (_isEditMode && widget.existingRecord?.id != null) {
        ...
        if (linked != null) {
          // It was recorded as a credit (reduces balance), so add it back.
  ...
  //     final totalDaysInMonth = DateTime(_selectedYear, _selectedMonth + 1, 0).day;
  //
  //     // ★ NEW — pass the transaction provider + ledger toggle/amount so the
  //     // linked ledger credit entry (by salaryRecordId) stays in sync:
  //     //  - amount updates in place if a linked entry already exists
  //     //  - gets created fresh if the toggle was just turned on
  //     //  - gets removed if the toggle is turned off
  //     final txnProvider = context.read<StaffTransactionProvider>();
  //
  //     await provider.updateFullSalary(
  //       docId: rec.id!,
  ...
  //       transactionProvider: txnProvider, // ★ NEW
  //       recordInLedger: _recordInLedger, // ★ NEW
  ...
  Future<void> _saveNew() async {
    ...
    try {
      ...
      // ★ FIX: Use current date as generatedDate
