...
class _StudentFormState {
  ...
  final TextEditingController registrationFeeCtrl;
  final TextEditingController monthlyFeeCtrl;
  final TextEditingController academyFeeCtrl;
  final TextEditingController nameCtrl;
  final TextEditingController rollNoCtrl;
  final TextEditingController cnicCtrl;
  ...
  _StudentFormState({AdmissionStudent? student})
      : data = student ?? AdmissionStudent(),
        ...
        monthlyFeeCtrl = TextEditingController(
            text: student?.monthlyFee?.toStringAsFixed(0) ?? ''),
        academyFeeCtrl = TextEditingController(
            text: student?.academyFee?.toStringAsFixed(0) ?? ''),
        nameCtrl = TextEditingController(text: student?.name ?? ''),
        rollNoCtrl = TextEditingController(text: student?.classRollNo ?? ''),
        cnicCtrl = TextEditingController(text: student?.bFormCnic ?? '');
  ...
  void syncFees() {
    ...
    data.registrationFee = double.tryParse(registrationFeeCtrl.text);
    data.monthlyFee = double.tryParse(monthlyFeeCtrl.text);
    data.academyFee = double.tryParse(academyFeeCtrl.text);
    data.classRollNo =
    rollNoCtrl.text.trim().isEmpty ? null : rollNoCtrl.text.trim();
    data.bFormCnic =
    cnicCtrl.text.trim().isEmpty ? null : cnicCtrl.text.trim();
...
class _AdmissionFormScreenState extends State<AdmissionFormScreen> {
  ...
  Future<void> _fetchSchoolFees(int idx) async {
    final form = _studentForms[idx];
    if (form.data.classId == null) return;
    setState(() => form.loadingFees = true);
    try {
      final fees = await context
          .read<AdmissionProvider>()
          .fetchFees(form.data.classId!, form.data.sectionName);
      setState(() {
      ...
      setState(() => form.loadingFees = false);
      _snack('Failed to load school fees');
    }
  }

  Future<void> _fetchAcademyFees(int idx) async {
    final form = _studentForms[idx];
    if (form.data.academyId == null) return;
    setState(() => form.loadingFees = true);
    try {
      final fees = await context
          .read<AdmissionProvider>()
          .fetchAcademyFees(form.data.academyId!);
      setState(() {
        form.academyFeeCtrl.text =
            fees['monthlyFee']?.toStringAsFixed(0) ?? '';
        form.data.academyFee = fees['monthlyFee'];
        form.loadingFees = false;
      });
    } catch (e) {
      setState(() => form.loadingFees = false);
      _snack('Failed to load academy fees');
    }
  }
  ...
  Future<void> _save() async {
    ...
    for (int i = 0; i < _studentForms.length; i++) {
      final f = _studentForms[i];
      if (f.nameCtrl.text.trim().isEmpty) continue;
      if (!f.data.hasSchool && !f.data.hasAcademy) {
        _snack('Please select School or Academy for Student ${i + 1}');
        return;
      }
      if (f.data.hasSchool && f.data.classId == null) {
        _snack('Please select a class for Student ${i + 1}');
        return;
      }
    ...
    _familyIdDebounce?.cancel();
    _familySearchDebounce?.cancel();
    for (final f in _studentForms) f._debounce?.cancel();
    ...
    return Column(
      ...
      children: [
        ...
        Row(
          children: [
            Expanded(
                child: _field(_fatherNameCtrl, 'Father Name *', Icons.person)),
            const SizedBox(width: 12),
            Expanded(
              child: _field(
                  _fatherOccCtrl,
  ...
  Future<void> _loadAllFamilies() async {
    ...
    try {
      final families =
      await context.read<AdmissionProvider>().fetchAllFamilies();
      if (mounted) {
        setState(() {
          _allFamilies = families;
          _loadingAllFamilies = false;
  ...
  Future<void> _loadAcademies() async {
    if (_academies.isNotEmpty) return;
    setState(() => _loadingAcademies = true);
    try {
      final list =
      await context.read<AdmissionProvider>().fetchAcademies();
      if (mounted) {
        setState(() {
          _academies = list;
      ...
      if (mounted) {
        ...
        _snack('Failed to load academies');
      }
    }
  }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                Expanded(
                  child: CheckboxListTile(
                    ...
                    onChanged: (val) {
                      setState(() {
                        form.data.hasSchool = val ?? false;
                        if (!form.data.hasSchool) {
                          form.data.classId = null;
                          form.data.className = null;
                          form.data.sectionId = null;
                          form.data.sectionName = null;
                          form.annualFeeCtrl.clear();
                          form.registrationFeeCtrl.clear();
                          form.monthlyFeeCtrl.clear();
                        }
                      });
                    },
                  ),
                ...
                Expanded(
                  child: CheckboxListTile(
                    ...
                    onChanged: (val) {
                      setState(() {
                        form.data.hasAcademy = val ?? false;
                        if (!form.data.hasAcademy) {
                          form.data.academyId = null;
                          form.data.academyName = null;
                          form.data.academyFee = null;
                          form.academyFeeCtrl.clear();
                        } else {
                          _loadAcademies();
                        }
                      });
            ...
            if (!form.data.hasSchool && !form.data.hasAcademy)
              Padding(
                padding: const EdgeInsets.only(top: 4),
                child: Text(
                  'Select at least School or Academy',
            ...
            if (form.data.hasSchool) ...[
              ...
              const SizedBox(height: 10),
              _buildSchoolFeesCard(idx, isPre),
            ],

            if (form.data.hasAcademy) ...[
              const SizedBox(height: 10),
              // DropdownButtonFormField<String>(
              //   value: form.data.academyId,
              //   decoration: const InputDecoration(
              //     labelText: 'Academy *',
              //     border: OutlineInputBorder(),
              //     prefixIcon: Icon(Icons.school),
              //   ),
              //   items: _loadingAcademies
              //       ? [
              ...
              //   onChanged: (val) async {
              //     final selected =
              //     _academies.firstWhere((a) => a.id == val);
              //     setState(() {
              //       form.data.academyId = val;
              //       form.data.academyName = selected.name;
              //     });
              //     await _fetchAcademyFees(idx);
              //   },
              //   validator: (v) =>
              //   v == null ? 'Select academy' : null,
              // ),
              const SizedBox(height: 10),
              TextFormField(
                controller: form.academyFeeCtrl,
                decoration: const InputDecoration(
                  labelText: 'Academy Fee (Rs)',
                  border: OutlineInputBorder(),
                  prefixText: 'Rs ',
      ...
      children: [
        ...
        if (sections.isNotEmpty) ...[
          ...
          DropdownButtonFormField<String>(
            ...
            decoration: InputDecoration(
              labelText:
              sectionRequired ? 'Section *' : 'Section (Optional)',
              border: const OutlineInputBorder(),
              prefixIcon: const Icon(Icons.group_outlined),
            ),
            items: sections
  ...
  Widget _buildSchoolFeesCard(int idx, bool isPre) {
    final form = _studentForms[idx];
    return Card(
      color: Colors.grey.shade50,
      elevation: 0,
          ...
          children: [
            Row(
              children: [
                ...
                Text(
                  isPre
                      ? 'School Fee (fetched from class, editable)'
