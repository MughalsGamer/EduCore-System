...
import '../providers/auth_provider.dart';
import 'admission mangement/admission_list_screen.dart';
import 'attendance_management/Attendance theme.dart';
import 'attendance_management/mark_attendance_calendar_screen.dart';
import 'attendance_management/mark_attendance_quick_screen.dart';
import 'attendance_management/view_edit_attendance_screen.dart';
import 'family_management/family management.dart';
import 'student_management/student_list.dart';
import 'teacher_management/teacher_list.dart';
import 'class_management/class_list.dart';
...
//  Constants
// ─────────────────────────────────────────────
const _purple = AttendanceTheme.primary;
const _purpleLight = AttendanceTheme.primaryLight;
...
class _DashboardScreenState extends State<DashboardScreen>
  ...
  void initState() {
    ...
    _screenBuilders = {
      ...
      'Profit / Loss': () => const ProfitLossScreen(),
      'Register User': () => const RegisterUserScreen(),
      // Attendance now lands directly on View/Edit (the list), which itself
      // offers a "Mark Attendance" entry point (Quick + Calendar) via a
      // bottom sheet / FAB, instead of showing an intermediate picker.
      //
      // showAppBar: false here because this builder is used for the wide
      // desktop layout's embedded right/main panel (which has its own
      // panel chrome). The mobile push path uses _pushMobileScreen below,
      // which wraps this in a Scaffold + AppBar with a back button so the
      // page isn't left without navigation chrome.
      'Attendance': () => const ViewEditAttendanceScreen(showAppBar: false),
      'Quick Attendance': () => MarkAttendanceQuickScreen(
        showAppBar: false,
        onSaved: () => _closeRightPanel(),
      ),
      'Calendar Attendance': () => MarkAttendanceCalendarScreen(
        showAppBar: false,
        onSaved: () => _closeRightPanel(),
      ),
      // Quick actions (only used in right panel)
  ...
  // ── Navigation items (all visible for now) ──
  // Screens whose builder always passes showAppBar: false (because they
  // double as embedded panels on desktop) need to be wrapped in a Scaffold
  // + AppBar when pushed as a standalone mobile page — otherwise the page
  // would have no title and no back button.
  static const _needsAppBarWrapperOnMobilePush = {'Attendance'};
  ...
  List<_NavItem> _navItems(String role) {
    ...
    final all = <_NavItem>[
      ...
      _NavItem('Teachers', Icons.person_rounded, go('Teachers')),
      _NavItem('Staff', Icons.badge_rounded, go('Staff')),
      // Attendance now opens the View/Edit list directly (dashboard → list),
      // matching every other nav item's behavior instead of a picker dialog.
  ...
  List<_QuickAction> _quickActions() {
    return [
      ...
      _QuickAction('Add Staff/Teacher', Icons.person_add_rounded,
              () => _openQuickAction('Add Staff/Teacher')),
      // Mark Attendance quick action now goes straight to the Attendance
      // screen (same destination as the sidebar item) rather than a picker.
      _QuickAction('Mark Attendance', Icons.fact_check_rounded, () {
        if (MediaQuery.of(context).size.width >= 700) {
          ...
          Navigator.push(
            context,
            MaterialPageRoute(
                builder: (_) => _pushMobileScreen('Attendance')),
          );
        }
      }),
    ];
  ...
  // ── Mobile bottom nav ──
  // Fixed: previously padded to 4 tabs with a dead "More" item that did
  // nothing when tapped. Now only real, working destinations are shown.
  Widget _buildBottomNav(String role) {
    final items = _navItems(role);
    final tabs = <_NavItem>[
