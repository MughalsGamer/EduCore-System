...
import 'package:educoresystem/screens/teacher_management/academy_staff_list_screen.dart'; // ★ NEW
import 'package:flutter/material.dart';
import 'package:provider/provider.dart';
import '../models/teacher.dart';
import '../providers/auth_provider.dart';
import '../providers/school_setting_prodvider.dart'; // ★ NEW
...
// ─────────────────────────────────────────────
//  Constants (Missing colors fixed here)
// ─────────────────────────────────────────────
...
class _DashboardScreenState extends State<DashboardScreen>
  ...
  void initState() {
    ...
    // ★ NEW — load + live-sync school settings (logo, name, etc.) so the
    // sidebar brand and AppBar always reflect what's saved in
    // School Settings, across the whole dashboard.
    ...
    _screenBuilders = {
      ...
      'Class Attendance': () => const ClassAttendanceScreen(showAppBar: false),   // ★ NEW
      'Class Attendance Report': () => const ClassAttendanceReportScreen(), // ★ NEW
      'Student Attendance Report': () => const StudentAttendanceReportScreen(),   // ★ NEW
      'Teachers': () => const TeacherListScreen(),
      'School Staff': () => const StaffListScreen(), // ★ RENAMED (was 'Staff')
      'Academy Staff': () => const AcademyStaffListScreen(), // ★ NEW
      ...
      // ✅ FIXED: Removed old Quick/Calendar screens to fix "Undefined" errors
  ...
  // ✅ FIXED: Removed 'Attendance' from wrapper so AppBar doesn't double
  ...
  Widget _pushMobileScreen(String label) {
    ...
    return Scaffold(
      backgroundColor: Colors.grey.shade50, // ✅ FIXED: Replaced AttendanceTheme
      appBar: AppBar(
        title: Text(label),
  ...
  List<_NavItem> _navItems(String role) {
    ...
    final all = <_NavItem>[
      ...
      _NavItem('Salary List', Icons.list_alt_rounded, go('Salary List')),
      _NavItem('Salary Adjustment', Icons.payments_rounded, go('Salary Adjustment')),
  ...
  List<_QuickAction> _quickActions() {
    return [
      ...
      _QuickAction('Add Transaction', Icons.add_card_rounded,
              () => _openQuickAction('Add Transaction')),

      // ✅ FIXED: Directly call the new screen without wrapper conflicts
      _QuickAction('Mark Attendance', Icons.fact_check_rounded, () {
        if (MediaQuery.of(context).size.width >= 700) {
          ...
          Navigator.push(
            context,
            MaterialPageRoute(builder: (_) => const AttendanceScreen()), // Direct push
          );
        }
      }),
    ];
  ...
  Widget _sidebarContent(String role, String userEmail, {required bool isDrawer}) {
    ...
    // ★ NEW — live school settings for brand block
    ...
    if (schoolSettings.logoBase64 != null &&
        schoolSettings.logoBase64!.isNotEmpty) {
      try {
    ...
    return Column(
      ...
      children: [
        // Brand — ★ now shows live logo + school name from School Settings
        Padding(
          ...
          child: Row(
            children: [
              Container(
                ...
                height: 50,
                decoration: BoxDecoration(
                  color: logoImage != null ? Colors.transparent : _purple,
        ...
        Expanded(
          child: ListView(
            ...
            children: [
              ...
              ...items.where((n) => n.label == 'Attendance').map((e) => _sbTile(e, isDrawer)),
              if (items.any((n) => [
                'Register User', 'School Staff', 'Academy Staff', 'Teachers', 'ID Cards'
              ].contains(n.label))) ...[
                _sbLabel('People'),
                ...items
                    .where((n) => [
              ...
              // Settings group, appears at the very bottom of the nav list
  ...
  Widget _buildBottomNav(String role) {
    ...
    final tabs = <_NavItem>[
      ...
      items.firstWhere((n) => n.label == 'Fee Receipts'),
    ...
    return BottomNavigationBar(
      ...
      onTap: (i) {
        ...
        if (i == 0) return; // Dashboard tab is the current screen already
        tabs[i].onTap();
      },
  ...
  Widget build(BuildContext context) {
    ...
    // ★ NEW — live school settings for AppBar title/logo
    final schoolSettings = context.watch<SchoolSettingsProvider>().settings;
    final schoolNameForAppBar = schoolSettings.schoolName.trim().isEmpty
        ? 'Citizens Model School'
        : schoolSettings.schoolName;
          ...
          children: [
            if (isWide) ...[
              Container(
                ...
                height: 34,
                decoration: BoxDecoration(
                  color: appBarLogoImage != null ? Colors.transparent : _purple,
                  borderRadius: BorderRadius.circular(6),
                  image: appBarLogoImage != null
                      ? DecorationImage(image: appBarLogoImage, fit: BoxFit.cover)
                      : null,
                ),
                child: appBarLogoImage == null
                    ? const Icon(Icons.school_rounded,
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  Text(schoolNameForAppBar, // ★ live school name
