...
class _StudentWiseResultCardsScreenState
  ...
  Future<void> _confirmAndGeneratePdf(
    ...
    try {
      ...
      // Convert to pairs for result entry builder
      final pairs = selected
          .map((r) => MapEntry(r.examCard, r.studentMarks))
          .toList();

      // Prepare entries (percentage & positions computed quickly)
      final entries = buildResultCardEntries(
        selected: pairs,
        studentLookup: lookup,
      );

      // Generate PDF – this is the heavy part but now faster with pw.Page
      final bytes = await ResultCardPdfGenerator.generate(results: entries);

      if (!mounted) return;

      // Show the print dialog (still faster than before because generation is quick)
