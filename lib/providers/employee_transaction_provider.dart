...
//   }
//
//   // Get employee ledger with running balance
//   Future<Map<String, dynamic>> getEmployeeLedger(String employeeId) async {
//     final transactions = await _getAllTransactionsForEmployee(employeeId);
//
//     double balance = 0;
...
//   }
//
//   // ★ NEW — find a single transaction (for this employee) that is linked
//   // to a given salaryRecordId. Used to keep the salary-generated ledger
//   // credit entry in sync whenever the salary record is edited/deleted.
//   Future<StaffTransaction?> getTransactionBySalaryRecordId({
//     required String employeeId,
//     required String salaryRecordId,
//   }) async {
//     try {
//       final snap = await _collection
//           .where('employeeId', isEqualTo: employeeId)
//           .where('salaryRecordId', isEqualTo: salaryRecordId)
//           .limit(1)
//           .get();
...
//       return StaffTransaction.fromMap(doc.data() as Map<String, dynamic>, doc.id);
//     } catch (e) {
//       debugPrint('Error finding transaction by salaryRecordId: $e');
//       return null;
//     }
//   }
//
//   // Add transaction with balance calculation
//   Future<void> addTransaction(StaffTransaction transaction) async {
//     _saving = true;
//     notifyListeners();
//
...
//   }
//
//   // Add credit transaction from salary deduction
//   Future<void> addSalaryDeduction({
//     required String employeeId,
//     required String employeeName,
//     required String employeeType,
...
//   }
//
//   // ★ NEW — Create, update, or remove the ledger credit entry that is
//   // linked to a specific salary record (via salaryRecordId), so the
//   // "Deduct from Balance" toggle on the salary screen always stays in
//   // sync with the ledger — whether the salary is newly generated,
//   // edited (amount/date/note/toggle changed), or the toggle is turned off.
//   //
//   // Behavior:
//   //  - recordInLedger == false  -> delete any existing linked transaction
//   //  - recordInLedger == true & no existing linked transaction -> create one
//   //  - recordInLedger == true & existing linked transaction found -> update
//   //    its amount/date/note in place (keeps the same transaction id, so
//   //    the ledger doesn't show duplicate/orphan entries)
//   Future<void> syncSalaryDeduction({
//     required String employeeId,
//     required String employeeName,
//     required String employeeType,
//     required String salaryRecordId,
//     required bool recordInLedger,
//     required double amount,
//     required DateTime date,
//     required String note,
//   }) async {
//     final existing = await getTransactionBySalaryRecordId(
//       employeeId: employeeId,
//       salaryRecordId: salaryRecordId,
//     );
//
//     if (!recordInLedger) {
//       // Toggle is off — remove any linked ledger entry if present.
//       if (existing?.id != null) {
...
//     }
//
//     // Toggle is on but amount is 0/not entered — treat same as "off"
//     // (nothing meaningful to record), and clean up any stale entry.
//     if (amount == 0) {
//       if (existing?.id != null) {
//         await deleteTransaction(existing!.id!);
//       }
...
//
//     if (existing == null) {
//       // No linked entry yet — create a fresh credit transaction.
...
//       );
//     } else {
//       // Linked entry already exists — update it in place (same id),
//       // recalculating running balances across the ledger.
...
//   }
//
//   // ★ NEW — remove the ledger credit entry linked to a salary record.
//   // Used when the salary record itself is deleted.
//   Future<void> deleteSalaryDeductionForRecord({
//     required String employeeId,
//     required String salaryRecordId,
//   }) async {
//     final existing = await getTransactionBySalaryRecordId(
